//! Utilities for viewing trees of mixed [crate::forest::basic_indirect::BasicNode] and [crate::forest::chunk::UniformChunk].
//! where the children of the basic nodes are [ChunkId]s which correspond to either a chunk, or a BasicNode.
//!
//! The [crate::forest::Node] implemented here is abstracts this as children are which are either Nodes, or ChunkIds.
...
//! is done by [crate::forest::indirect_nav] which wraps this node in a Node implementation up with a forest using [crate::forest::nav::Nav].

use crate::forest::{indirect_node::IndirectChunk, uniform_chunk::UniformChunk};
...
#[apply(crate::forest::enum_node::fromMembers!)]
pub enum enum_chunk {
